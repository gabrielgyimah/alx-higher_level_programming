...
class Square:
    ...
    def __init__(self, size=0, position=(0, 0)):
        """Initialize instances of class Sqaure"""
    ...
    @property
    def size(self):
        """Returns the szie of the current object"""
        return self.__size

    @size.setter
    def size(self, size):
        """Sets the value of size for the current object"""
        if not isinstance(size, int):
            raise TypeError("size must be an integer")
        if size < 0:
            raise ValueError("size must be >= 0")
    ...
    @property
    def position(self):
        """Returns the field position value"""
        return self.__position

    @position.setter
    def position(self, position):
        """Sets the field position value"""
        if len(position) != 2 or not isinstance(position, tuple):
            raise TypeError("position must be a tuple of 2 positive integers")
